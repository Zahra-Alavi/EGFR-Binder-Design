...
class AF2_runner():
    ...
    def generate_scoredict(self, feat_holder, confidences, rmsds) -> None:
        ...
        # Extract the iPTM and pTM scores
        iptm_score = confidences.get('iPTM', float('nan'))
        ptm_score = confidences.get('pTM', float('nan'))
    ...
    def process_output(self, feat_holder, feature_dict, prediction_result) -> None:
        ...
        # Extract the predicted aligned error logits and breaks
        pae_logits = prediction_result['predicted_aligned_error']['logits'][...]
        pae_breaks = prediction_result['predicted_aligned_error']['breaks'][...]


        # Generate the asym_id array: residues belonging to different chains should have different IDs
        num_residues = len(feat_holder.seq)
        asym_id = np.ones(num_residues, dtype=int)

        # If the structure is not a monomer, set the asym_id for the second chain
        if not feat_holder.monomer:
            asym_id[feat_holder.binderlen:] = 2  # Assuming the binder and target are separate chains


        # Ensure asym_id is a numpy array of integers
        asym_id = np.asarray(asym_id, dtype=int)

        # Compute the pTM score
        ptm_score = confidence.predicted_tm_score(
          logits=pae_logits,
          breaks=pae_breaks
        )   

        # Compute the iPTM score using the asym_id
        try:
            iptm_score = confidence.predicted_tm_score(
                logits=pae_logits,
                breaks=pae_breaks,
                asym_id=asym_id,
                interface=True
            ...
            print(f"Error calculating iPTM score: {e}")
            iptm_score = float('nan')




        # Store both pTM and iPTM in the confidences dictionary
        confidences['pTM'] = ptm_score
        confidences['iPTM'] = iptm_score

     
        print(f"TM Scores: {ptm_score, iptm_score}")
